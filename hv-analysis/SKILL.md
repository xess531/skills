...
开始搜索前，先探测当前 Agent 运行环境中有哪些信息获取能力可用。**不要假设任何特定平台或目录结构**——不同 Agent 框架（WorkBuddy、Claude Code、Cursor、Windsurf、自建 Agent 等）的工具组织方式各不相同。
...
使用子Agent并行搜索。每个子Agent的prompt中应包含上一步探测到的可用工具清单，让子Agent直接使用，不要让它再花时间自己发现。
...
**学术类研究对象额外要求**：如果研究对象涉及学术概念、算法、AI模型、技术范式，子Agent必须查询 arXiv（`https://export.arxiv.org/api/query?search_query=all:关键词&max_results=10`）获取相关论文。

子Agent的prompt应描述目标（"获取""调研""了解"），不要用暗示具体手段的动词（"搜索""爬取"），让子Agent自主判断最佳获取方式。
